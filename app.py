...
def create_grout_intake_comparison(data, hole_id, stage):
    try:
        ...
        if all_stages_intake.empty:
            raise ValueError("No data available for grout intake comparison")
...
# Define the missing apply_tma function
def apply_tma(data, window=3):
    try:
        data['flow_tma'] = data['flow'].rolling(window=window, min_periods=1).mean()
        data['effPressure_tma'] = data['effPressure'].rolling(window=window, min_periods=1).mean()
        return data
    except Exception as e:
        log_error(f"Error applying TMA: {e}")
        return data
